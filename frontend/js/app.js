// ======================================================================
//  FULL UPDATED frontend/js/app.js
//  - External Chart.js tooltip (description first)
//  - Thin left color bar (A1) for each player row
//  - Sticky behavior: tooltip remains when mouse enters tooltip box
//  - Pulsate + fade logic preserved (Option C behavior)
//  - All other features preserved for compatibility with your backend
// ======================================================================
...
const selectedMultipleHolder = document.getElementById(
  "selectedMultipleHolder"
);
...
suggestions.addEventListener("click", async (e) => {
  ...
  try {
    const r = await fetch(
      `/api/player?player_id=${encodeURIComponent(selectedPlayerId)}`
    );
    const j = await r.json();
...
function renderInputPlayer(p) {
  const html = `
    ...
    <div class="stat-row"><span class="stat-key">Matches</span><span class="stat-value">${p["Playing Time MP"]}</span></div>
    <div class="stat-row"><span class="stat-key">Goals</span><span class="stat-value">${p["Performance Gls"]}</span></div>
    <div class="stat-row"><span class="stat-key">Assists</span><span class="stat-value">${p["Performance Ast"]}</span></div>`;
...
//
// RADAR RENDER (external tooltip: description first, then player list with left color bar)
//
function renderRadar(inputRadar, players, inputName) {
  ...
  // external tooltip function (description first, then list of players)
  function externalTooltip(context) {
    const { chart, tooltip } = context;
    const tooltipEl = document.getElementById("customTooltip");
    if (!tooltipEl) return;

    // If no tooltip data, schedule hide (unless pointer is inside tooltip or canvas)
    if (tooltip.opacity === 0) {
      // If pointer inside tooltip or canvas, keep visible
      if (pointerInsideTooltip || pointerInsideCanvas) {
        return;
      }
      // schedule hide shortly to allow entering tooltip
    ...
    // require dataIndex from tooltip.dataPoints[0]
    const dataIndex =
    ...
    // Build rows: include every dataset (including input dataset at index 0)
    const rows = chart.data.datasets.map((ds, dsIndex) => {
      const val =
        ds.data && ds.data[dataIndex] !== undefined ? ds.data[dataIndex] : "-";
      // prefer original border color for the left bar; fallback to borderColor
      const color = ds._origBorderColor || ds.borderColor || "rgba(0,0,0,1)";
      const name = ds.label || `Series ${dsIndex + 1}`;
      return { name, val, color };
    });

    // Compose HTML: description first
    let html = `<div class="tt-title">${label}</div>`;
    if (desc && desc.trim().length > 0) {
      html += `<div class="tt-desc">${desc}</div>`;
      html += `<div style="height:10px;"></div>`; // small spacer
    }

    html += `<div class="tt-sep"></div>`; // visual separator (CSS will style)
    html += `<div class="tt-rows">`;
    rows.forEach((r) => {
      const displayVal =
        r.val === null || r.val === undefined || r.val === "" ? "-" : r.val;
      // left thin color bar is rendered by CSS with an inline background-color for the bar
      html += `
        <div class="tt-row">
    ...
    // Positioning: use tooltip.caretX/Y inside canvas; fallback to center
  ...
  canvasEl.addEventListener("mouseleave", () => {
    ...
    // schedule hide if tooltip not hovered
    clearTimeout(hideTooltipTimeout);
    hideTooltipTimeout = setTimeout(() => {
      if (!pointerInsideTooltip && !pointerInsideCanvas) {
        customTooltip.style.display = "none";
  ...
  // Add tooltip box hover listeners to support sticky mode
  ...
  customTooltip.addEventListener("mouseleave", () => {
    ...
    // if not inside canvas either, hide after short delay
    clearTimeout(hideTooltipTimeout);
    hideTooltipTimeout = setTimeout(() => {
      if (!pointerInsideTooltip && !pointerInsideCanvas) {
        customTooltip.style.display = "none";
  ...
  // Reset pulsation state
  stopPulsate();
}

//
// PULSATE + FADE LOGIC (Option C behavior)
//
...
function startPulsate(playerRk) {
  ...
  // fade others (input dataset index 0 unchanged)
...
async function compareMultiple(ids) {
  try {
    ...
    aiReportBox.innerHTML = `<pre style="white-space:pre-wrap;">${j.ai_report}</pre>`;
...
function renderCompareBoxes(j) {
  ...
  inputPlayerContent.innerHTML = `
    <div class="stat-row"><span class="stat-key">Player</span><span class="stat-value">${
      j.players[0].Player
    }</span></div>
    ${keys
      .filter((k) => k !== "Player")
      ...
      <div class="stat-row">
        ...
        <span class="stat-value">${
          inp.stats[k] === undefined ? "" : inp.stats[k]
        }</span>
      </div>
    `
      )
    ...
    <div class="compare-box">
      ...
      ${keys
        ...
        <div class="stat-row">
          <span class="stat-key" data-key="${k}">${k}</span>
          <span class="stat-value">${
            row.stats[k] === undefined ? "" : row.stats[k]
          }</span>
        </div>
      `
        )
  ...
  if (showAllFeatures) {
    ...
    inputPlayerContent.innerHTML =
      ...
      Object.keys(p0)
        .filter((k) => k !== "Rk")
        .map((k) => {
          return `<div class="stat-row"><span class="stat-key" data-key="${k}">${k}</span><span class="stat-value">${
            p0[k] === undefined ? "" : p0[k]
          }</span></div>`;
        })
        .join("");
